...
Fig2B <- LM.avis %>% 
   ...
   ylab("Tree height (m)") +
   stat_smooth(method = 'nls', geom="line", alpha=0.9, 
               size=1, span=0.7 ,                # Fit nls model
               method.args = list( formula = 'y~H*(1-exp(-a*x))',  
                                   start=c(H=20, a=0.2)), se=FALSE)+
   scale_linetype_manual(name="Condition", values = c("solid", "longdash" ),
...
#### Fig 2c ####
Fig2C <- LM.avis %>% 
   ggplot(., aes(x=DBH, y=slenderness,fill = Condition,
                 linetype=Condition,
