...
links2$tnames2 <- paste(links2$LOC, links2$Tree2) #unique identifier for each tree T1 combining plot and tree ID


######### Create network ############

g<-graph(c(links2$tnames,links2$tnames2))
...
plot(a_bs_pl$bootstraps[,2], a_bs_pl$bootstraps[,3],xlab = "K_min",ylab = "gamma")
...
allPlot1<-ggplot(data=a_df_bs_pl, aes(x=xmin, y=pars)) + labs(x="K_min", y="gamma") + theme_bw() + 
  ...
  annotate("text", x=K.min_D.min, y=min(a_df_bs_pl$pars), label=K.min_D.min, col="blue") +
  annotate("text", x=min(a_df_bs_pl$xmin), y=gamma_D.min, label=round(gamma_D.min, digits=2), col="blue")


D.min <- d_est[which.min(d_est$D), 3]

ggplot(data=a_df_bs_pl, aes(gof)) + geom_histogram() + labs(x="D", y="frequency") + geom_vline(xintercept=D.min, colour="red") + theme_bw()
...
a_bs_pl_sat_cut <- bootstrap_p(m_pl, xmins = pairs[which.min(pairs$D), 1], xmax = pairs[which.min(pairs$D), 2], no_of_sims = 5000, threads = 2)
